...
<div class="row row-cols-md-2 row-cols-1">
    <div class="col mb-4">
        <div class="bg-white-custom p-4 card-setting">
            <h4 class="title text-dark-custom mb-4">Địa chỉ</h4>
            <div class="body">
                <form action="{{ route('admin.setting.store') }}" method="post" enctype="multipart/form-data">
                    @csrf

                    @php
                        $address = json_decode($footer['FooterAddresses'] ?? '[]', true);
    ...
    <div class="col mb-4">
        <div class="bg-white-custom p-4 card-setting">
            <h4 class="title text-dark-custom mb-4">Một số nội dung</h4>
            <div class="body">
                <form action="{{ route('admin.setting.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="item mb-4">
                        <x-form.input_text 
                            label="Giờ mở cửa" 
                            name="FooterHouseOpen"
                            value="{{ $footer['FooterHouseOpen'] ?? '' }}" />

                        <x-form.input_text 
                            label="Góp ý khiếu nại" 
                            name="FooterComplaints"
                            value="{{ $footer['FooterComplaints'] ?? '' }}" />

                        <x-form.input_text 
                            label="Slogan" 
                            name="FooterSlogan"
                            value="{{ $footer['FooterSlogan'] ?? '' }}" />
                    </div>
                    <x-button type="submit" label="Cập nhật" style="margin-left: auto" />
                </form>
            </div>
