    ...
    const fetchBlogDetail = async () => {
      try {
        ...
        if (data?.status === "success" && data.data) {
          // Normalize image URLs for blog and related products
          const normalizedBlog = {
            ...data.data,
            image_url: data.data.image_url.startsWith("/")
              ? data.data.image_url
              : `/${data.data.image_url}`,
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div
                className={`grid gap-6 ${
                  blog.products.length === 1
                    ? "grid-cols-1 max-w-sm mx-auto"
                    : blog.products.length === 2
                    ? "grid-cols-1 md:grid-cols-2 max-w-2xl mx-auto"
                    : blog.products.length === 3
